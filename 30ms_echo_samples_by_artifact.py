...
def sample(inputs):
    ...
    for i, n in enumerate(ind['d']):
        ...
        # params[:,n].fill_(1.0)
    # for i, c in zip(range(0,6),[0.25,0.50,0.75,1.0,1.25,1.50]):
    for i, c in zip(range(0,6),[0.0,0.25,0.50,1.0,1.50,2.0]):
        params[i,ind['d']] *= c
        params[i,ind['g']] = 0
    '''
    Should I zero out the gaussian values? If so, sshould I get rid of the lorentzian values for all other samples?
    '''
    # else:
    ...
    for i, c in zip(range(samples_per_group*cnt,samples_per_group*(cnt+1)),[x for x in torch.linspace(1, 5000**0.5, samples_per_group)]):
    # for i, c in zip(range(6,12),[x for x in torch.linspace(1, 50, samples_per_group)]):
        params[i,ind['g']] = c #if i<10 else c/2
        for _, n in enumerate(ind['d']):
            params[i,n] = 0
    for n in ind['g']:
        if n>0 and n<l-g-1:
    ...
    for n in ind['g']: params[12:,n] = 20.0
    
    
    if config.lineshape in ['voigt']:
        pass
    cnt += 1
    print('cnt: ',cnt)
    d = [0.1, 1.0, 2.0]
    ...
    # print("params[36:39,ind['f_shift']]: ",params[36:39,ind['f_shift']])
       
    params[samples_per_group*cnt+3:,ind['f_shift']].fill_(0.0)
    
    
    for i in range(samples_per_group*cnt+3,samples_per_group*(cnt+1)):
        for n in ind['f_shifts']:
        # sz = params[38:,n].unsqueeze(-1).size()
            params[0:samples_per_group*cnt+3,n] = 0.0
            params[i,n] = torch.distributions.uniform.Uniform(-.10,.10).sample([1])
            
    for n in ind['f_shifts']:
        params[samples_per_group*(cnt+1):,n].fill_(0.0)
